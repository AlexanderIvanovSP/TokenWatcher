...
#ifndef COMMONPKCS11_H
#define COMMONPKCS11_H
...
)
{
	...
	count = strlen(header) // Место под начальный тег
	...
	*result = (char*)calloc(count, sizeof(char));
	...
	//компоновка данных
	strcat_s(*result, sizeof(char) * count, header);
	for (i = 0; i < length; i += width) {
		strncat_s(*result, sizeof(char) * count, buffer + i, width);
		strcat_s(*result, sizeof(char) * count, "\n");
	}
	strcat_s(*result, sizeof(char) * count, footer);
